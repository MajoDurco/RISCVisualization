...
export function execute(instruction, registers, ui, pipeline){
  ...
  if(is_jumping){
    ...
    ui.addTo(`${instruction.instruction} instruction has calculated that ${instruction.params[FIRST_OPERAND]}(${first_op}) is less then ${instruction.params[SECOND_OPERAND]}(${second_op}) so jump is going to be executed in next stage to  line ${dest}`, 'state_line_msg')
    pipeline.stacks.branch_stack.push(dest)
  }
  else { // condition of false
    ui.addTo(`${instruction.instruction} instruction has calculated that ${instruction.params[FIRST_OPERAND]}(${first_op}) is NOT less then ${instruction.params[SECOND_OPERAND]}(${second_op}) so jump is NOT going to be executed`, 'state_line_msg')
  }
}
